...
const applyFilters = (overrides = {}) => {
    router.get(route('categories.show', props.category.slug), {
        ...
        ...overrides,
    }, { preserveState: true, preserveScroll: true, replace: true, only: ['listings', 'filters', 'favoriteListingIds', 'stats', 'counties', 'facets'] });
};
...
const favoritedIds = computed(() => new Set(props.favoriteListingIds));

const similarCategories = computed(() => {
    const groups = groupCategories(props.categories);
    const group = groups.find((g) => g.categories.some((c) => c.slug === props.category.slug));
    const pool = group ? group.categories : props.categories;
    return pool.filter((c) => c.slug !== props.category.slug).slice(0, 6);
});
</script>
                    ...
                    <div class="relative mt-7 flex flex-wrap gap-2.5">
                        <div class="flex min-w-[260px] flex-1 items-center gap-2.5 rounded-full border border-ivt-line bg-white p-1 pl-4 shadow-ivt-soft">
                            ...
                            <input
                                v-model="search"
                                type="text"
                                :placeholder="`Caută după nume sau zonă — ex. „Studio Lumina”…`"
                ...
                <div class="mx-auto grid max-w-7xl grid-cols-1 gap-9 px-6 lg:grid-cols-[272px_1fr] lg:px-8">
                    ...
                    <div>
                        ...
                        <div class="mb-7 flex flex-wrap items-center justify-between gap-4">
                            <p class="text-[14.5px] text-ivt-ink-soft"><b class="text-ivt-ink">{{ listings.total }}</b> furnizori găsiți</p>
                            <select
                                :value="sort"
                                @change="setSort"
                        ...
                        <!-- VENDOR GRID -->
                        <div v-if="listings.data.length" class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            <VendorCard
